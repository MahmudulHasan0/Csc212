//works for 4,5,6, 7 has a issue with the usedcol
...
{ 
    ...
    { 
     ...
     //IF current POS VIOLATE RULES, SHIFT IT BY 1
        if (usedCol[curr] || prev==curr || prev==curr+1 || prev==curr-1) 
        {
            ...
            { 
                ...
                queenPos.pop(); 
                N++; //bec i poped i need to take care of one more queen  from *N--;
             ...
             //(1) TEST TO SEE IF ITS NEAR ANY OTHER QUEENS OR IS IN THE SAME COLUMN OR IS IN THE FORBIDEN ZONE
                if (queenPos.top()!=lastQueen && queenPos.top()+1!=lastQueen && queenPos.top()-1!=lastQueen && !usedCol[lastQueen] && lastQueen!=0 && lastQueen!=QUEEN+1)
            ...
            cout<<"-------------------------------------------------------------VIOLATE: N: "<<N<<" | counter: "<<counter<<" | stack.top: "<<queenPos.top()<<" "<<prev<<" | curr: "<<curr<<endl;         
     ...
     //IF NO VIOLATION, SET THE curr QUEEN ON THE BOARD
        ...
        cout<<"----------------------------------------------------------------------N: "<<N<<" | counter: "<<counter<<" | stack.top: "<<queenPos.top()<<" "<<prev<<" | curr: "<<curr<<endl;          print(queenPos, QUEEN);
    }
    // print(queenPos, QUEEN);
    return 0;
}
